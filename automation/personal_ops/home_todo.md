...
- 후속 확인 필요: agent_models.json(로컬 Gemma 설정)이 실제로 쓰이는 도구인지,
  ...
  -> 확인됨 (아래 9): 집에 Gemma 설치돼있고 실제로 씀. _company/ 유지, 기억형 역할 확정.

## 9. 멀티트랙 사업화 전략 확정 (2026-06-12)
- 핵심 사업진단 OS(CHEONOK_REVENUE_TARGET_OS.md)와 "병행"으로 드랍쉬핑/유튜브채널/온라인사업
  여러가지를 동시에 사업화 트랙으로 추진하기로 함 (단일 포커스 권고를 오버라이드)
- _company/_shared/goals.md에 "신규 트랙" 섹션 추가 -> 집의 Gemma 7-에이전트(현빈=Business,
  레오=YouTube)가 이 방향으로 우선순위 잡음
- 리스크 룰(보장형 매출약속/가짜증빙/투자자문 금지 등)은 신규 트랙에도 동일 적용
